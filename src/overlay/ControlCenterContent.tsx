...
function ToggleRow({
  ...
  return (
    <label className="flex items-center gap-2 rounded-lg px-2.5 py-2">
      <Icon className={`h-3.5 w-3.5 shrink-0 ${checked ? "text-foreground" : "text-muted-foreground"}`} />
  ...
  return (
    <div className="w-[400px] overflow-hidden rounded-xl border border-border bg-background p-1.5">
      ...
      <div className="mt-2 max-h-[220px] overflow-y-auto">
        <SectionHeader label="Open tabs" open={openTabsExpanded} onToggle={() => setOpenTabsExpanded((v) => !v)} />
        {openTabsExpanded &&
        ...
        <SectionHeader label="Recently closed" open={closedExpanded} onToggle={() => setClosedExpanded((v) => !v)} />
        {closedExpanded &&
          (filteredClosed.length === 0 ? (
            <p className="px-2.5 py-3 text-center text-[13px] text-muted-foreground">No recently closed tabs</p>
          ) : (
      ...
      <div className="max-h-[420px] overflow-y-auto pb-1">
        ...
        <div className="grid grid-cols-2 gap-0.5">
          <ToggleRow icon={ShieldBan} label="Ad blocker" checked={cc.adBlockEnabled} onChange={(v) => set({ adBlockEnabled: v })} />
          <ToggleRow icon={EyeOff} label="Do Not Track" checked={cc.doNotTrack} onChange={(v) => set({ doNotTrack: v })} />
          <ToggleRow icon={FileCode2} label="Disable JavaScript" checked={cc.javascriptDisabled} onChange={(v) => set({ javascriptDisabled: v })} />
          <ToggleRow icon={Cookie} label="Block cookies" checked={cc.cookiesBlocked} onChange={(v) => set({ cookiesBlocked: v })} />
          <ToggleRow icon={MousePointerSquareDashed} label="Block popups" checked={cc.popupBlock} onChange={(v) => set({ popupBlock: v })} />
          <ToggleRow
            icon={PlayCircle}
            ...
            {THROTTLE_OPTIONS.map((opt) => (
              <button
                ...
                className={`rounded-full px-2 py-0.5 text-[11px] font-medium transition-colors ${
                  cc.networkThrottle === opt.id ? "bg-foreground text-background" : "bg-muted text-muted-foreground hover:bg-muted/70"
                }`}
              >
                {opt.label}
              </button>
        ...
        <div className="grid grid-cols-2 gap-0.5">
          <ToggleRow icon={Video} label="Block camera" checked={cc.cameraGlobalBlock} onChange={(v) => set({ cameraGlobalBlock: v })} />
          <ToggleRow icon={Mic} label="Block microphone" checked={cc.micGlobalBlock} onChange={(v) => set({ micGlobalBlock: v })} />
          <ToggleRow icon={MapPin} label="Block location" checked={cc.locationGlobalBlock} onChange={(v) => set({ locationGlobalBlock: v })} />
        </div>

        <CategoryLabel label="VPN & security" />
        <div className="grid grid-cols-2 gap-0.5">
          <ToggleRow icon={ShieldCheck} label="VPN (Tor)" checked={cc.vpnEnabled} onChange={(v) => set({ vpnEnabled: v })} />
          <ToggleRow icon={Wifi} label="DNS over HTTPS" checked={cc.dnsOverHttpsEnabled} onChange={(v) => set({ dnsOverHttpsEnabled: v })} />
        </div>

        <CategoryLabel label="Performance" />
        <div className="grid grid-cols-2 gap-0.5">
          <ActionButton icon={ListX} label="Unload tab" onClick={() => act({ type: "unloadTab" })} />
          <ActionButton icon={ListX} label="Unload background tabs" onClick={() => act({ type: "unloadAllBackgroundTabs" })} />
          <ToggleRow
            icon={Cpu}
        ...
        <div className="grid grid-cols-2 gap-0.5">
          <ToggleRow icon={VolumeX} label="Master mute" checked={cc.masterMute} onChange={(v) => set({ masterMute: v })} />
          <ToggleRow icon={Moon} label="Force dark mode" checked={cc.darkModeForced} onChange={(v) => set({ darkModeForced: v })} />
          <ToggleRow icon={Focus} label="Focus mode" checked={cc.focusMode} onChange={(v) => set({ focusMode: v })} />
        </div>
        <div className="flex items-center gap-2 px-2.5 py-1.5">
          <span className="text-[12.5px] font-medium text-foreground">Font size / zoom</span>
          <div className="ml-auto flex items-center gap-1">
            <button
              onClick={() => set({ globalZoomFactor: Math.max(0.5, Math.round((cc.globalZoomFactor - 0.1) * 10) / 10) })}
              className="flex h-6 w-6 items-center justify-center rounded-full bg-muted hover:bg-muted/70"
            >
              <ZoomOut className="h-3 w-3" />
            </button>
            <span className="w-9 text-center text-[11px] tabular-nums text-muted-foreground">{zoomPct}%</span>
            <button
              onClick={() => set({ globalZoomFactor: Math.min(2, Math.round((cc.globalZoomFactor + 0.1) * 10) / 10) })}
              className="flex h-6 w-6 items-center justify-center rounded-full bg-muted hover:bg-muted/70"
            >
        ...
        <div className="grid grid-cols-2 gap-0.5">
          <ActionButton icon={Code2} label="Open DevTools" onClick={() => act({ type: "openDevTools" })} />
          <ActionButton icon={RotateCcw} label="Reload without cache" onClick={() => act({ type: "reloadNoCache" })} />
          <ActionButton icon={Trash2} label="Clear cache" onClick={() => act({ type: "clearCache" })} />
          <ActionButton icon={Camera} label="Screenshot" onClick={() => act({ type: "screenshot" })} />
          <ActionButton icon={QrCode} label="QR code of URL" onClick={() => setShowQr((v) => !v)} />
          <ActionButton icon={Printer} label="Print / Save as PDF" onClick={() => act({ type: "printPdf" })} />
        </div>
        {showQr && activeTab && !activeTab.isHome && !activeTab.isSettings && (
          <div className="flex flex-col items-center gap-1.5 px-2.5 py-2">
            ...
            />
            <span className="max-w-[300px] truncate text-[11px] text-muted-foreground">{activeTab.url}</span>
          </div>
        )}
        <div className="flex items-center gap-2 px-2.5 py-1.5 text-muted-foreground">
          <AlertTriangle className="h-3.5 w-3.5 shrink-0" />
          <span className="text-[11.5px]">Console errors (all tabs): {payload.consoleErrorTotal}</span>
        </div>
      </div>
    </div>
  );
