...
>(({ className, overlayClassName, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        // Was "sm:rounded-lg" — Tailwind's sm: breakpoint (640px) never
        // fires inside the small, fixed-size native overlay windows this
        // renders in (see ProfilePopupContent's remove-profile dialog),
        // so the corners rendered dead square there. Unconditional
        // rounded-lg (matches the app's own --radius) fixes every Dialog
        // usage, not just that one call site.
