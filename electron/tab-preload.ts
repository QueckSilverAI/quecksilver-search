...
if (process.isMainFrame) {
  ...
  // untouched — we only ever take over the page-level scrollbar. Left in
  // place unconditionally now (see the file header above for why pages that
  // style their own ::-webkit-scrollbar no longer get an exception).
  webFrame.insertCSS(`
...
function findScrollTarget(): ScrollTarget {
  ...
  if (docEl.scrollHeight > docEl.clientHeight + 1) return { el: docEl, isDocument: true };
...
function findLargestScrollableElement(): HTMLElement | null {
  ...
  for (const el of candidates) {
    ...
    // Only the primary content region counts — must cover at least half the
    // viewport in both dimensions, or a small inner scroll box (a comment
    // list, a code snippet) would win over the real page-level content.
...
function buildScrollbar(target: ScrollTarget) {
  ...
  const scrollEventTarget: Window | HTMLElement = target.isDocument ? window : scrollEl;
  ...
  if (!target.isDocument) {
    ...
    // marker attribute + a dedicated <style> rule, since inline styles
    // can't target the ::-webkit-scrollbar pseudo-element.
