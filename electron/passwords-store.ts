...
// Includes tombstones — only for supabase-sync.ts's merge step (see
// sync-merge.ts for why a deletion needs to survive as a tombstone rather
// than just disappearing from the array).
export function listStoredPasswordsForSync(windowId: number): StoredPasswordEntry[] {
  return store.read(windowId, []);
}

export function overwriteStoredPasswords(windowId: number, merged: StoredPasswordEntry[]) {
...
export function autoSaveFromForm(windowId: number, url: string, username: string, password: string): PasswordEntry | null {
  ...
  const existingIdx = entries.findIndex((e) => !e.deletedAt && hostnameOf(e.url) === targetHost && e.username === username);
...
export function removePassword(windowId: number, id: string) {
  // Soft delete (tombstone) rather than actually removing the entry — see
  // sync-merge.ts: a linked device that hasn't synced since can only tell
  // this was deleted (rather than "never existed here") if the tombstone
  // is still there to merge against. listPasswords() already filters these
  // out, so nothing in the UI ever sees a "deleted" password again.
  const entries = store.read(windowId, []);
  const idx = entries.findIndex((e) => e.id === id);
  if (idx === -1) return;
  entries[idx] = { ...entries[idx]!, deletedAt: Date.now(), updatedAt: Date.now() };
  store.write(windowId, entries);
}
...
export function importPasswords(windowId: number, imported: { url: string; username: string; password: string }[]): { imported: number; skipped: number } {
  const entries = store.read(windowId, []);
  const existingKeys = new Set(entries.filter((e) => !e.deletedAt).map((e) => `${e.url}\u0000${e.username}`));
  let importedCount = 0;
  let skipped = 0;
  const now = Date.now();
